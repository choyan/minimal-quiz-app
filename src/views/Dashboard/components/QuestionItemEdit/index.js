...
export default function QuestionItemEdit({ register, watch, index, id }) {
  ...
  const answers = watch(`questionList[${index}].answers`);
  const answerType = watch(`questionList[${index}].answer_type`);

  useEffect(() => {
    if (answers) {
      setCountAnswer(answers.length);
    }
  }, []);
